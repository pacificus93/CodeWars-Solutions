...
will use an IN to check whether a department has had a sale with a price over 90.00 dollars 
...
WITH special_sales AS (SELECT DEPARTMENT_ID FROM SALES WHERE PRICE > 90.00)

SELECT dp.* FROM DEPARTMENTS dp WHERE dp.ID IN (SELECT * FROM special_sales)