...
def compute_depth(n):
    ...
    # Following function turns integer into the set of digits that appear in the integer
    
    int_to_list = lambda x : set([i for i in list(str(x))])
    
    while len(list(ints)) < 10:
        cnt += 1
        ints.update(int_to_list(n*cnt))
